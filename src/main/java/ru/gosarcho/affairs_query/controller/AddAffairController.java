...
public class AddAffairController {
    ...
    public String saveAffair(Model model, @ModelAttribute("affairForm") AffairForm affairForm, HttpSession session) {
        ...
        if (fond != null && fond.length() > 0 && op != null && op.length() > 0 && affair != null && affair.length() > 0) {
            ...
            if (affairDir.exists()) {
                return addAndReturn(new AffairModel(fond, op, affair),Arrays.asList(Objects.requireNonNull(affairDir.listFiles())),session.getId());
            }
            ...
            if (matchingAffairs != null && matchingAffairs.length > 0) {
                return addAndReturn(new AffairModel(fond, op, affair), Arrays.asList(matchingAffairs),session.getId());
            }
    ...
    private String addAndReturn(AffairModel affairModel, List<File> affairList, String sessionId) {
        persons.get(sessionId).getAffairModels().add(affairModel);
        persons.get(sessionId).getAffairFiles().addAll(affairList);
